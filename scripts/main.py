#!/usr/bin/env python3
"""DASTツールのリリース情報を収集してフィードを生成するメインスクリプト。"""
import logging
import os
import tempfile
...
FEED_TITLE = "DAST Tools Feed"
FEED_BASE_URL = os.environ.get("FEED_BASE_URL", "https://tmyymmt.github.io/dast-tools-feed/feeds")
...
def collect_tool(tool: dict, github_token: Optional[str]) -> List[ReleaseEntry]:
    ...
    try:
        ...
        elif tool_type == "scrape_burpsuite":
            return collect_burpsuite()
        else:
...
<body>
  <h1>DAST Tools Feed</h1>
  <p>Release information feeds for DAST (Dynamic Application Security Testing) tools.</p>
...
def write_pages(tools: list, entries_by_tool: Dict[str, List[ReleaseEntry]]) -> None:
    ...
    write_file_atomic(
        ...
        render_html("DAST Tools Comparison", generate_comparison_page(tools, entries_by_tool)).encode("utf-8"),
    )
    write_file_atomic(
        PAGES_DIR / "comparison_ja.html",
        render_html("DASTツール比較", generate_comparison_page_ja(tools, entries_by_tool), lang="ja").encode("utf-8"),
    )
