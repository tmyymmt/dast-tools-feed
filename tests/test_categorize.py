...
def test_hotfix_burpsuite():
    assert classify_release("[Hotfix] critical patch", "", "scrape_burpsuite") == "security"


def test_pricing_keyword_japanese():
    assert classify_release("料金プランの変更について", "", "scrape_burpsuite") == "pricing"
...
def test_announcement_burpsuite():
    assert classify_release("Security Days 2024 出展のお知らせ", "", "scrape_burpsuite") == "announcement"


def test_regular_release_burpsuite_defaults_to_feature():
    assert classify_release("定期リリース", "バグ修正と機能追加", "scrape_burpsuite") == "feature"
