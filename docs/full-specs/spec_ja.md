...
- 費用表示。`tools.yml` に `community_url` が設定され、かつ `pricing` に `Free (Community)` を含む場合、`Free (Community)` は公式 Community Edition ページへのリンクとして表示する。`pricing_url` が設定され、かつ `pricing` に `Paid` を含む場合、`Paid` は公式有償版ページへのリンクとして表示する。
- **機能表**：9 項目の機能フラグを ✅/❌ で表示
...
概要版テーブルの費用列では、`community_url` が設定されており `pricing` に `Free (Community)` を含む場合、その文字列を公式 Community Edition ページへのリンクとして表示する。`pricing_url` が設定されており `pricing` に `Paid` を含む場合、`Paid` は公式有償版ページへのリンクとして表示する。
